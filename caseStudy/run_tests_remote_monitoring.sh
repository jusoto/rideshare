...
if [[ (-z "$times") || (-z "$test_cases") || (-z "$host") || (-z "$filename") || (-z "$log_filename") ]]
  then
    echo "
  ERROR:
  ...
  Usage: run_test_remote_monitoring.sh 100 1,2,3,4,5 localhost output_filename log_filename
...
TEST_CASES_SCRIPT="/lclhome/jsoto128/Documents/Projects/FIU/paper/rideshare_zipkin/caseStudy/test_cases.sh"
...
run_remote_tests() {
    ...
    ssh jsoto128@kirby $TEST_CASES_SCRIPT  $times "$test_cases" $host $filename
    #sshpass -p 'JuanPablo1706' ssh jsoto128@kirby 'bash -s' < $TEST_CASES_SCRIPT micros1 10000 "1,2,3,4,5" micros1_05012024_test_case
}
